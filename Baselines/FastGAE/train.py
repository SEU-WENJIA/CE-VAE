...
flags.DEFINE_string('dataset', 'ba3k_16', 'Name of the graph dataset')
''' Available datasets:
...
flags.DEFINE_integer('nb_node_samples', 300, 'Number of nodes to sample at each \
...
for i in range(FLAGS.nb_run):
    ...
    if FLAGS.fastgae:
        ...
        sampled_nodes, adj_label, adj_sampled_sparse = node_sampling(adj, node_distribution,
                                                       FLAGS.nb_node_samples, FLAGS.replace,FLAGS.measure)
    else:
    ...
    for iter in range(FLAGS.iterations):
        ...
        if FLAGS.fastgae:
            ...
            # New node sampling
            sampled_nodes, adj_label, _ = node_sampling(adj, node_distribution,FLAGS.nb_node_samples, FLAGS.replace,FLAGS.measure)
...
if FLAGS.task == 'link_prediction':
    ...
    print("Mean AUC score: ", np.mean(mean_roc),
          "\nStd of AUC scores: {:.8f}".format(np.std(mean_roc)), "\n \n")


    print("AP scores\n", mean_ap)
    print("Mean AP score: ", np.mean(mean_ap),
          "\nStd of AUC scores: {:.8f}".format( np.std(mean_ap)), "\n \n")
